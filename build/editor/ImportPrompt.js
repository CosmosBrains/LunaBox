...
export class ImportPrompt {
    constructor(_doc) {
        ...
        this._importButton = button({ style: "border-image-source: none !important; height: auto; min-height: var(--button-size); margin: 0.5em; width:50%; color: var(--primary-text); border-bottom: solid; border-bottom-color: var(--link-accent);" }, "Import");
        this._exportButton = button({ style: "border-image-source: none !important; height: auto; min-height: var(--button-size); margin: 0.5em; width:50%; color: var(--secondary-text);" }, "Export");
        ...
        this._modeImportSelect = select({ style: "width: 100%;" }, option({ value: "auto" }, "Auto-detect mode (for json)"), option({ value: "BeepBox" }, "BeepBox"), option({ value: "ModBox" }, "ModBox"), option({ value: "JummBox" }, "JummBox"), option({ value: "SynthBox" }, "SynthBox"), option({ value: "GoldBox" }, "GoldBox"), option({ value: "PaandorasBox" }, "PaandorasBox"), option({ value: "UltraBox" }, "UltraBox"));
        this._importPrompt = div({}, div({ class: "promptTitle" }, h2({ class: "importExt", style: "text-align: inherit;" }, ""), h2({ class: "importTitle" }, "Import")), p({ style: "text-align: left; margin: 0.5em 0;" }, "BeepBox songs can be exported and re-imported as .json files. You could also use other means to make .json files for BeepBox as long as they follow the same structure."), p({ style: "text-align: left; margin: 0.5em 0;" }, "BeepBox can also (crudely) import .mid files. There are many tools available for creating .mid files. Shorter and simpler songs are more likely to work well."), this._modeImportSelect, this._fileInput, this._cancelButton);
        this._exportPrompt = div({ style: "display:none;" }, this.exportStuff._exportPrompt);
        this.customContainer = div({ class: "customContainer", id: "customContainer", style: "width: 100%;" }, p({ style: "text-align: center; margin: 1em 0; display:flex; flex-direction: row;" }, this._importButton, this._exportButton), this._importPrompt, this._exportPrompt);
        this.container = div({ class: "prompt noSelection", style: "width: 300px;" }, this.customContainer);
        ...
        this.cleanUp = () => {
            ...
            window.removeEventListener("dragenter", (event) => { event.preventDefault(); });
            window.removeEventListener("dragleave", (event) => { event.preventDefault(); });
