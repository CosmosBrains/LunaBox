...
export class TipPrompt {
    constructor(_doc, type) {
        ...
        switch (type) {
            ...
            case "instrumentType":
                {
                    message = div(h2("Instrument Type"), p("LunaBox comes with many instrument presets, try them out! You can also create your own custom instruments!"), p("There are also options for copying and pasting instrument settings and for generating random instruments at the top of the instrument type menu."));
                }
                break;
            case "eqFilter":
                {
            ...
            case "chipWave":
                {
                    message = div(h2("Chip Wave"), p("LunaBox comes with some sound waves based on classic electronic sound chips, as well as several unique waves. This is the basic source of the sound of the instrument, which is modified by the other instrument settings."));
                }
                break;
            case "chipNoise":
                {
                    message = div(h2("Noise"), p("LunaBox comes with several basic noise sounds. These do not have any distinct musical pitch, and can be used like drums to create beats and emphasize your song's rhythm."));
                }
                break;
            ...
            case "effects":
                {
                    message = div(h2("Effects"), p("LunaBox has many different kinds of special effects you can add to instruments. You can turn on multiple effects at once, and they can be configured individually. Try them all out!"));
                }
                break;
            ...
            case "aliases":
                {
                    message = div(h2("Aliasing"), p("LunaBox applies a technique called 'anti-aliasing' to instruments normally to help them sound cleaner even at high frequencies and low sample rates."), p("When this setting is ticked that technique is disabled, so you may hear strange audio artifacts especially at high pitches and when bending notes. However, this can lend a grungy sound to an instrument that could be desirable."));
                }
                break;
            ...
            case "unisonSpread":
                {
                    message = div(h2("Unison Spread"), p("This setting controls the distance between the two voices, in semitones. A small amount of spread causes the voice's waves to shift in and out from each other, causing a shimmering effect. Larger spread will cause the voices to act like separate notes."), p("This setting will only work correctly with two voices."));
                }
                break;
            case "unisonOffset":
                {
            ...
            case "unisonExpression":
                {
                    message = div(h2("Unison Expression"), p("This setting controls the unison volume. Use this if the unison makes your instrument too loud in comparison to other instruments."));
                }
                break;
            case "unisonSign":
                {
            ...
            case "pitchRange":
                {
                    message = div(h2("Pitch Envelope Start and End"), p("These two settings will adjust where the start and end of the pitch envelope affects. Everything below start envelope will be 0, everything above end envelope will be 1, and everything inbetween will scale linearly based on pitch (the opposite is true if inverted)."), p("This will NOT work properly if pitch start is greater than pitch end."), p("These values are different than the MIDI numbers. These correspond to how many paino keys from the bottom of the song player a specific pitch is"));
                }
                break;
            case "envelopeInvert":
                {
                    message = div(h2("Envelope Inversion"), p("This setting will invert the envelope curve. So instead of, for example, lower pitches leading to a smaller output, lower pitches can lead to a greater output."));
                }
                break;
            case "envelopeRange":
                {
                    message = div(h2("Envelope Bounds"), p("These two settings stretch or shrink the envelope vertically, allowing for different ranges of affect."), p("This will NOT work properly if lower bound is greater than upper bound."));
                }
                break;
            case "modEnvelope":
                {
                    message = div(h2("Envelope Target"), p("This setting specifies which envelope of the specified instrument you would like to change."));
                }
                break;
            case "randomSteps":
                {
                    message = div(h2("Random Envelope Steps"), p("This setting changes how many \"steps\", or different possible values can be outputted. For example, a step size of 2 will output either 0 or 1, and a step size of 3 either 0, 0.5, or 1. Every step is equidistant from each other"));
                }
                break;
            case "randomSeed":
                {
                    message = div(h2("Random Envelope Seed"), p("There are 64 seeds, or pseudorandom patterns that you can choose from when enveloping a setting."), p("The same seed will output the same value per tick or pitch if the other envelope settings are also the same, meaning that if two different songs use the same seed for their envelope they will have the same \"randomization\"."));
                }
                break;
            case "lfoEnvelopeWaveform":
                {
                    message = div(h2("LFO Envelope Waveform"), p("LFO envelopes can output a variety of different waveforms, from old tremolo's sine to more complex ones."), p("These waves are: sines, squares, triangles, and sawtooths."));
                }
                break;
            case "randomEnvelopeType":
                {
                    message = div(h2("Random Envelope Type"), p("Random Envelopes can switch between being determined by the time in the song, the pitch of the note, or per note trigger."));
                }
                break;
            case "ringMod":
                {
                    message = div(h2("Ring Modulation"), p(`This setting multiplies a selected wave's frequency with an instrument frequency, this is useful for "bell-like" instruments.`));
                }
                break;
            case "RingModHz":
                {
                    message = div(h2("Ring Modulation (Hertz)"), p(`This setting changes the Hertz of the multiplied frequency.`), p(`The offset allows you to increment the Hertz by 1.`));
                }
                break;
            case "phaserMix":
                {
                    message = div(h2("Phaser Mix"), p(`This setting adds holes (aka notches) or peaks in the frequency spectrum to the waveform its given. The placement of these peaks and notches in the waveform can be changed by using envelopes and/or modulators.`));
                }
                break;
            case "phaserFreq":
                {
                    message = div(h2("Phaser Frequency"), p(`This setting controls the frequency of the the peaks and notches of the phaser.`));
                }
                break;
            case "phaserFeedback":
                {
                    message = div(h2("Phaser Feedback"), p(`This setting effects how pronounced the Feedback of the Phaser is, this is done by adding the Feedback of the Phaser back into itself.`));
                }
                break;
            case "phaserStages":
                {
                    message = div(h2("Phaser Stages"), p(`This slider changes how many all-passes there are in the phaser. An all-pass is created by splitting an audio signal into paths, every path after the first one that is created is called an all-pass.`), p(`The minimum value of this slider is 1, the reason being that 1 all-pass is the exact same as having no phaser at all, if there were 0 all-passes then the audio would cease to exist.`));
                }
                break;
            case "unisonBuzzes":
                {
                    message = div(h2("Unison Buzzing"), p("This setting controls whether or not there's a buzzing noise when there's only 1 unison voice, and the spread is set to something other than 0."));
                }
                break;
            case "slideSpeedSlider":
                {
                    message = div(h2("Slide Speed"), p("This slider controls how fast/slow the slide transition is. In other audio software, this setting is sometimes referred to as \"Portamento\"."));
                }
                break;
            case "strumSpeedSlider":
                {
                    message = div(h2("Strum Speed"), p("This setting affects how fast your chord will strum."));
                }
                break;
            case "upperNoteLimit":
                {
                    message = div(h2("Upper Note Limit"), p("Defines the upper limit in which notes will play, useful for advanced instruments."));
                }
                break;
            case "lowerNoteLimit":
                {
                    message = div(h2("Lower Note Limit"), p("Defines the lower limit in which notes will play, useful for advanced instruments."));
                }
                break;
            case "granular":
                {
                    message = div(h2("Granular Synthesis"), p(`This effect is based on granular synthesis! It takes random points from a wave and rearranges them to form "sonic clouds".`), p(`This particular slider controls the wet/dry mix of the granulation.`));
                }
                break;
            case "grainSize":
                {
                    message = div(h2("Grain Size"), p(`This setting controls the size of the grain.`));
                }
                break;
            case "grainAmount":
                {
                    message = div(h2("Grain Freq"), p(`This setting controls about how often a grain (a group of audio samples) is added to the output, from rarely to multiple at once.`));
                }
                break;
            case "grainRange":
                {
                    message = div(h2("Grain Range"), p(`This setting controls the range of randomization for grain sizes. `));
                }
                break;
            default:
