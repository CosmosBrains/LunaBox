...
const createElement = (tagName, className, content, attributes = {}, ariaLabel) => {
  ...
  ObjectForEach(isString(ariaLabel) ? {
    ...attributes,
    "aria-label": ariaLabel
  } : attributes, (name, value) => element.setAttribute(name, value + ""));
  return element;
};
...
const getBoundingRectArray = element => {
  const {
    x,
    y,
    width,
...
const stringify = (color, format = RGB_FORMAT) => format ? format + (format === RGB_FORMAT ? `(${color.r}, ${color.g}, ${color.b}` : `(${color.h}, ${color.s}%, ${color.l}%`) + (color.a < 1 ? `, ${color.a})` : ")") : DEFAULT_COLOR;
...
const getFormatFromObj = value => isPlainObject(value) && [HSL_FORMAT, RGB_FORMAT].find(format => [...format].every(channel => isNumber(value[channel]))) || "";
const normalizeColor = value => {
  return isString(value) ? value : stringify(value, getFormatFromObj(value));
};
...
const parseColor = (color, opacity) => {
  ...
  if (/^hsl/.test(colorStr)) {
    const [isHSL, h, angle, s, l, a = "1", percentage] = HSL_REGEX.exec(colorStr) || [];
    if (isHSL) {
      hsl = {
...
const isLabeledSwatch = value => isPlainObject(value) && "color" in value;
const Swatches = alwan => {
  let isCollapsed = false;
  const renderSwatch = (swatch, labelTemplate) => {
    let button;
    let color;
    let colorStr;
    let label;
    ...
    colorStr = normalizeColorStr(colorStr) === DEFAULT_COLOR ? DEFAULT_COLOR : colorStr;
    label = isString(label) ? label : colorStr;
    button = createButton(labelTemplate.replace("%label%", label), "alwan__swatch", "", label);
  ...
  return {
    _render({
      ...
      container = createDivElement(swatches.map(swatch => renderSwatch(swatch, buttons.swatch)), "alwan__swatches");
      if (!toggleSwatches) {
        return container;
      }
...
const createPopover = (target, floating, containingBlock, ref, {
  margin,
  position,
  closeOnScroll,
  toggle,
  ...
  const updatePosition = () => {
    ...
    translate(floating, ...coordinates.map((value, axis) => {
      ...
      (viewport[axis] - floatingRect[axis + 2]) / 2) - containingBlockRect[axis]);
    }));
  };
  if (toggle) {
  ...
  const updatePositionOnScroll = ({
    ...
    if (shadowRoot instanceof ShadowRoot && t.contains(shadowRoot.host) || t.contains(target)) {
      updatePosition();
      if (closeOnScroll) {
        _toggle(false);
      }
  ...
  return {
    _setup(options) {
      ...
      if (popover) {
        ...
        popoverContainingBlock = createDivElement(root, "alwan__popover-container");
        popoverInstance = createPopover(targetEl, root, popoverContainingBlock, ref, config, self);
      } else {
      ...
      parentEl ? parentEl.append(popoverContainingBlock) : targetEl.after(popoverContainingBlock);
      if (disabled) {
